...
class Tencent
{
     ...
     * @return string
     */
    public static function api($memberId,$command, $params = array(), $method = 'GET', $multi = false)
    {
        if (isset($_SESSION['access'][$memberId])) {//OAuth 2.0 方式
            //鉴权参数            
            $params['access_token'] = $_SESSION['access'][$memberId]['t_access_token'];
            $params['oauth_consumer_key'] = OAuth::$client_id;
            $params['openid'] = $_SESSION['access'][$memberId]['t_openid'];
        ...
        } elseif (isset($_SESSION['access'][$memberId]['t_openid']) && isset($_SESSION['access'][$memberId]['t_openkey'])) {//openid & openkey方式
            $params['appid'] = OAuth::$client_id;
            $params['openid'] = $_SESSION['access'][$memberId]['t_openid'];
            $params['openkey'] = $_SESSION['access'][$memberId]['t_openkey'];
            $params['clientip'] = Commons::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
