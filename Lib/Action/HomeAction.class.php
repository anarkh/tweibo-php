...
class HomeAction extends Action {
	...
	if ($result) {
		...
		$re = Tencent::api($memberId, 'user/info'); //调用腾讯微博api获得用户信息               
...
//	if (isset($_FILES["attach"]) && isset($_POST['picMemberId'])) {
//	    if ((($_FILES["attach"]["type"] == "image/gif") || ($_FILES["attach"]["type"] == "image/jpeg") || ($_FILES["attach"]["type"] == "image/jpg") || ($_FILES["attach"]["type"] == "image/png") || ($_FILES["attach"]["type"] == "image/bmp") || ($_FILES["attach"]["type"] == "image/ico")) && ($_FILES["attach"]["size"] < 4194304)) {
...
//		} else {
//		    $pic = $_FILES["attach"]["tmp_name"];
//		    $multi = array('pic' => $pic);
//		    $params = array(
//			'pic_type' => 2,
//			 "pic_url" => ""			
//		    );
//		    $memberId = $_POST['picMemberId'];
//		    $result = Tencent::api($memberId, 't/upload_pic', $params, 'POST', $multi);
//		    $r = json_decode($result, true);
//		    if ($r['ret'] === 0) {
//			echo '<body onload="showPic()"><script type="text/javascript">function showPic(){parent.show("' . $r['data']['imgurl'] . '")}</script></body>';
//		    } else {
//			echo '<body onload="showPic()"><script type="text/javascript">function showPic(){parent.showTipsMsg("444' . $memberId. '")}</script></body>';
//		    }
//		}
//	    } else {
//		echo '<body onload="showPic()"><script type="text/javascript">function showPic(){parent.showTipsMsg("图片类型错误或者图片过大")}</script></body>';
//	    }
//	} else {
//	    echo '<body onload="showPic()"><script type="text/javascript">function showPic(){parent.showTipsMsg("1231313' . $_POST['picMemberId'] . '")}</script></body>';
//	}
//    }
    //显示上传的图片
    public function showPic() {
	...
	require_once CONF_PATH . 'emotionJson.php';
